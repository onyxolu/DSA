class Solution:
    #Time complexity: O(V + E). Because for a graph of V = N nodes. There can be max N-1 eges assuming no cycles. Since this implementation handles cycles as well. Hence, O(N)
    #Space: O(N)
    #optimise using a dijoint set and union-find algorithm
    def __init__(self):
        self.flights = dict()
