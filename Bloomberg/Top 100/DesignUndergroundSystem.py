...
class UndergroundSystem:
    ...
    def checkIn(self, id: int, stationName: str, t: int) -> None:
        # for checkin, key = id, val = (stationName, time)
        self.checkIns[id] = (stationName, t)

    def checkOut(self, id: int, stationName: str, t: int) -> None:
        # for check out, key = (startStation, stationName), val = (totalTime, noOfTrips)
        startStation, startTime = self.checkIns[id]
        totalTime, trips = self.checkOuts[(startStation, stationName)]
        self.checkOuts[(startStation, stationName)] = (
            totalTime + (t - startTime), trips + 1)
