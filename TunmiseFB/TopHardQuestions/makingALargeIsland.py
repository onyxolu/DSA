...
Time Complexity: O(N^4), where N is the length and width of the grid. For a general graph with m edges and n vertices, DFS from each node is O(m+n). In this case, m=O(n) so each DFS is O(n^2). We perform DFS on O(n^2) nodes so the total time is O(n^4).
